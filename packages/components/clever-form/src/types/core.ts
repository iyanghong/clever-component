...
/** 表单字段值类型 */
export type FormFieldValue = string | number | boolean | Date | null | undefined | any[]
...
export interface FormFieldSchema<T = Record<string, any>> {
  ...
  // 事件回调
  /** 值变化回调 */
  onChange?: (value: any, oldValue: any, formData: T, methods: FormMethods<T>) => void | Promise<void>
  /** 失焦回调 */
...
/** 表单配置 */
export interface FormConfig<T = Record<string, any>> extends Omit<NFormProps, 'model'> {
  /** 表单字段配置 */
